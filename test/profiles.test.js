...
test("profile save uses an upsert and returns the saved profile", async () => {
  ...
  const res = response();
  await createProfileHandlers(database).save({ params: { userId: "7" }, auth: { userId: 7 }, body: { nickname: "MeepleFan", imageUrl: "https://example.com/avatar.png" } }, res);
...
test("profile image URL must use HTTP or HTTPS", () => {
  assert.throws(() => normalizeProfile({ imageUrl: "javascript:alert(1)" }), /HTTP\(S\)/);
});
